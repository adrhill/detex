"""Tests for the reduce_min propagation handler.

Tests min reduction along single and multiple dimensions,
full reductions, size-1 dimensions, and high-level functions
that lower to reduce_min.
"""
...
@pytest.mark.reduction
def test_reduce_min_1d_size_one():
...
@pytest.mark.reduction
def test_reduce_min_2d_axis0():
    """Reduce along axis 0: each output column depends on its column of inputs."""
    shape = (3, 4)

    def f(x):
        return lax.reduce_min(x.reshape(shape), axes=(0,))
...
@pytest.mark.reduction
def test_reduce_min_2d_axis1():
    """Reduce along axis 1: each output row depends on its row of inputs."""
    shape = (3, 4)

    def f(x):
        return lax.reduce_min(x.reshape(shape), axes=(1,))
...
@pytest.mark.reduction
def test_reduce_min_2d_both_axes():
    """Reduce along both axes: full reduction to scalar."""
    shape = (3, 4)

    def f(x):
        return lax.reduce_min(x.reshape(shape), axes=(0, 1))
...
@pytest.mark.reduction
def test_reduce_min_2d_size_one_reduced_dim():
...
@pytest.mark.reduction
def test_reduce_min_2d_size_one_kept_dim():
...
@pytest.mark.reduction
def test_reduce_min_3d_single_axis():
    """Reduce one axis of a 3D array."""
    shape = (2, 3, 4)

    def f(x):
        return lax.reduce_min(x.reshape(shape), axes=(1,)).flatten()
...
@pytest.mark.reduction
def test_reduce_min_3d_two_axes():
    """Reduce two axes of a 3D array."""
    shape = (2, 3, 4)

    def f(x):
        return lax.reduce_min(x.reshape(shape), axes=(0, 2)).flatten()
...
@pytest.mark.reduction
def test_reduce_min_3d_full():
    """Full reduction of a 3D array."""
    shape = (2, 3, 4)

    def f(x):
        return lax.reduce_min(x.reshape(shape), axes=(0, 1, 2))
...
@pytest.mark.reduction
def test_reduce_min_4d():
    """Reduce selected axes of a 4D array."""
    shape = (2, 2, 3, 2)

    def f(x):
        return lax.reduce_min(x.reshape(shape), axes=(1, 3)).flatten()
...
@pytest.mark.reduction
def test_reduce_min_then_reduce_min():
    """Chained reductions: min along axis 0 then axis 0 again."""
    shape = (2, 3, 4)

    def f(x):
        a = lax.reduce_min(x.reshape(shape), axes=(0,))  # (3, 4)
        return lax.reduce_min(a, axes=(0,))  # (4,)
...
@pytest.mark.reduction
def test_reduce_min_after_broadcast():
    """Reduce after broadcast: non-contiguous input dependencies."""

    def f(x):
        # x.shape = (3,), broadcast to (2, 3), then min along axis 0
        return lax.reduce_min(jnp.broadcast_to(x, (2, 3)), axes=(0,))
...
@pytest.mark.reduction
def test_reduce_min_after_reduce_sum():
    """Mix of reduce_sum then reduce_min."""
    shape = (2, 3, 4)

    def f(x):
        a = jnp.sum(x.reshape(shape), axis=2)  # (2, 3)
        return lax.reduce_min(a, axes=(1,))  # (2,)
