"""Propagation rule for reduce_min operations."""
...
def prop_reduce_min(eqn: JaxprEqn, deps: Deps) -> None:
    """Min reduction selects one element per reduction group.

    Any input in the group could be the minimum,
    ...
    Full reduction (all axes):
        out = min(x)  →  out depends on all inputs
    Partial reduction along axis k:
        out[i] = min_j x[i, j]  →  out[i] depends on all x[i, :]

    Example: y = min(x, axis=1) where x.shape = (2, 3)
